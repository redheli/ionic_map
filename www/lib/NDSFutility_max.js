...
function xy2ll(Form)
{
  // ori_lat,ori_lon,x,y,x_lat,y_lon
   with(Math)
   {
      /* get the origin lat and lon */
      //var oll=getoll(Form);
      olat=Form.ori_lat;//oll.olat;
      olon=Form.ori_lon;//oll.olon;
     ...
     /*
      ...
      {
         sx=1*Form.Xcord.value;
         sy=1*Form.Ycord.value;
      }
      */
     sx = Form.x;
     sy = Form.y;
     rotation_angle_degs = Form.rotation_angle_degs;

      var   origin={};

      origin={x:sx,y:sy,coord_system:1,olat:olat,
olon:olon,xoffset_mtrs:0,yoffset_mtrs:0,rotation_angle_degs:rotation_angle_degs,rms_error:0};
